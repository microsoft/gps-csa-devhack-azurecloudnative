...
-   <a href="cloud-native-applications---infrastructure-edition-hands-on-lab-step-by-step">云原生应用 - 开发人员版动手实验</a>
    ...
    -   <a href="#exercise-3-deploy-the-solution-to-azure-kubernetes-service">练习 3： 将解决方案部署到Azure Kubernetes 服务</a>
        ...
        -   <a href="#task-5-review-azure-monitor-for-containers">任务 5：审查容器的Azure监视器</a>
        -   <a href="#task-6-monitor-aks-with-azure-hosted-Prometheus-and-Grafana">任务 6：使用Auzre托管Prometheus和Grafana监控AKS</a>
    ...
    -   <a href="#exercise-5-working-with-services-and-routing-application-traffic">练习 5：练习5：使用Service，Ingress和Traffic Manager处理应用流量</a>
    ...
    -   <a href="#after-the-hands-on-lab">动手实验后考虑</a>
...
10. 启动 API 作为后端进程。
    ...
    ![In this screenshot, node ./server.js & has been typed and run at the command prompt, which starts the API as a background process.](media/image47.png "Start the mongodb in background")
...
13. 导航到 Web 应用程序目录，运行`npm install`和`ng build`.
    ...
    ![In this screenshot, after navigating to the web application directory, node ./server.js & has been typed and run at the command prompt, which runs the application as a background process as well.](media/image48.png "Running web server")
...
15. 从build machine 中的Cloud Shell使用vim编辑`app.js`文件。
    ...
    然后按下 **_i_** 进入编辑模式后，用build machine IP Address替换localhost。使用Address替换localhost。**使用localhost及build machine IP Address都可以使content-web应用在vm的运行环境下成功调用content-api。但在后面的章节我们将会把该应用运行环境biuld成docker镜像，进行容器化改造。该应用在容器环境里如果仅使用localhost的话，content-web将无法成功调用content-api。如果在容器环境里期望通过容器网络成功调用content-api，可以加载外部环境变量（比如使用docker -e参数加载CONTENT_API_URL=http://api:3001 ）或者如这个步骤更改的通过在app.js里将localhost更新成biuld vm的IP**
...
6.  克隆您的fabmedical存储库（用您的存储库的 URL 替换 下列URL）：
    ...
    ```bash
    git clone https://gitee.com/USER_NAME/fabmedical.git
    ```
...
25. 我们现在将我们的Helm Chart提交到我们的 Gitee 存储库。在"FabMedical"克隆的根文件夹中执行以下命令：
...
### <a id="task-5-review-azure-monitor-for-containers">任务 5：查看Azure Monitor 关于容器的信息</a>
...
### <a id="task-6-monitor-aks-with-azure-hosted-Prometheus-and-Grafana">任务 6：使用Auzre托管Prometheus和Grafana监控AKS</a>
...
2.  如果未注册，请使用以下命令注册：
    ...
    ![](.//media/image3.png)
3.  首先通过Azure门户创建**Azur Grafana托管服务**

![图形用户界面, 文本, 应用程序, 电子邮件描述已自动生成](.//media/image10.png)

4.  根据向导输入相关的参数，在本实验中使用你的资源组**fabmedical-xxx**,Azure Regioin选择 **East US **然后点击 **"审阅+创建"**

![图形用户界面, 文本, 应用程序描述已自动生成](.//media/image11.png)

5.  在创建好的Grafana托管服务的"概述"界面上，查看Grafana终结点，点击进去，查看Dashboard

![](.//media/image12.png)

6.  目前还没有集成Prometheus，还不能观察AKS指标。

![](.//media/image13.png)

7.  在Azure门户里，进入AKS集群管理界面的"见解"项，点击 **"启用Prometheus"**
...
9.  根据向导继续输入参数并集成之前创建的 **"Grafana工作区"**，之后点击 **"配置"**

![](.//media/image16.png)

10.  为收集AKS集群的监控指标，Prometheus会生成一个基于K8SDeamonSet的守护资源workload，即**ama-metrics-node**。通过以下命令验证该DeamonSet是否部署成功。
    ```bash
    kubectl get ds ama-metrics-node --namespace=kube-system
    ```
![文本 描述已自动生成](.//media/image17.png)

11.  到目前为止，Prometheus与Grafana在该AKS集群的集成、配置及启用已经完成。接下来从AKS监视页面点击"查看Grafana工作区"查看监控指标。

![图形用户界面, 应用程序描述已自动生成](.//media/image18.png)

12.  点击 **"浏览仪表板"**，之后根据您的Azure身份管理策略可能需要进行认证。

![图形用户界面, 应用程序描述已自动生成](.//media/image19.png)

13.  默认情况下，Managed Prometheus已经显示在Grafana Dashboards上，无需近一步配置数据源。在Managed Prometheus列点击"Go to folder ".

![电脑萤幕的截图描述已自动生成](.//media/image20.png)

14.  可监控的AKS资源类别列了出来，点击Pod来查看一下指标。

![](.//media/image21.png)

15.  Pod相关指标列了出来。Grafana可视化监控功能非常强大，可以近一步设置**Grafana Dashboards**，来客户化显示所需的指标。
...
## <a id="exercise-5-working-with-services-and-routing-application-traffic">练习5：使用Service，Ingress和Traffic Manager处理应用流量</a>

**期间**：1小时

在之前的练习中，我们对服务的Scale属性进行了限制。在本练习中，您将配置 api deployment，以创建使用动态端口映射来消除Scale活动期间端口资源限制的Pod。

Kubernetes 服务可以发现分配给每个Pod的端口，允许您在同一agent Node上运行多个Pod实例，---配置特定静态端口（如 API 服务的 3001）时无法实现的状态。
...
### <a id="task-3-perform-a-rolling-update">任务 3：执行滚动更新-选择性完成</a>
  &emsp;
 **注：本任务由于需要用到DevOps工具，例如使用Github Actions，Jenkins等，因此不是必须完成的。可以直接跳到下一个任务接着做实验。如果您想要完成本任务，可以参考[此处](https://gitee.com/help/articles/4230)**
...
6.  将这些更改推送至存储库，以便 Gitee结合CI工具（例如azure devops）生成和部署新的容器镜像。本实验不覆盖具体步骤，感兴趣可以参考[Gitee 如何使用 Azure PipeLine 构建项目](https://gitee.com/help/articles/4230)。
...
7.  访问`content-web`为您的 Gitee FabMedical存储库执行操作，并查看新镜像被部署到您的Kubernetes集群中。
...
## <a id="after-the-hands-on-lab">动手实验后考虑</a>

**时间**：10分钟
&emsp;

- 本devhack动手实验使用的应用后段数据持久化保存在数据库中，例如Azure Cosmos DB，应用层实现了无状态，如果有些应用场景需要在应用层Pod挂载持久化的Vollume，可以使用Azure Disk或者Azure File，这些持久化的数据可以在Pod重启后恢复。利用本实验的环境，可以继续尝试体验使用Azure Disk或者Azure File实现持久化数据。
- 参考动手实验文档：https://learn.microsoft.com/zh-cn/azure/aks/azure-disks-dynamic-pv
